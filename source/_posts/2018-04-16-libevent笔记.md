...
```c
int event_base_priority_init(struct event_base *base, int n_priorities);    // 1 ~ EVENT_MAX_PRIORITIES
...
- asynchronous-IO bufferevents (Windows only, experimental)
...
- fd 需要时 non-blocking 的，可以使用`evutil_make_socket_nonblocking()`。
- 不想现在指定 fd 可以将其设为 -1。
